...
def train(
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if save:
            ...
            torch.save(chkpt, latest,_use_new_zipfile_serialization=False)

            # Save best checkpoint
            if best_loss == test_loss:
                torch.save(chkpt, best, _use_new_zipfile_serialization=False)

            # Save backup every 10 epochs (optional)
            if epoch > 0 and epoch % 10 == 0:
                torch.save(chkpt, weights + 'backup%g.pt' % epoch,_use_new_zipfile_serialization=False)
